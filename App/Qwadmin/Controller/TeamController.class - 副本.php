...
class TeamController extends ComController
{
    ...
    public function edit()
    {
        ...
        if ($uid) {
            //$team = M('team')->where("uid='$uid'")->find();
            $prefix = C('DB_PREFIX');
            $user = M('team');
            $team = $user->field("{$prefix}team.*,{$prefix}auth_group_access.group_id")->join("{$prefix}auth_group_access ON {$prefix}team.uid = {$prefix}auth_group_access.uid")->where("{$prefix}team.uid=$uid")->find();
    ...
    public function update($ajax = '')
    {
        ...
        if (!$uid) {
            ...
            if (M('team')->where("user='$user'")->count()) {
                $this->error('用户名已被占用！');
            }
            $data['user'] = $user;
            $uid = M('team')->data($data)->add();
            ...
            M('team')->data($data)->where("uid=$uid")->save();
