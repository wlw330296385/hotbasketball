...
class LessonController extends ComController
{
    public function index()
    {
        $lesson = M('lesson')->select();
    ...
    public function update()
    {
        ...
        if ($data['title']) {
            ...
            $data['rules'] = $rules;
            if ($id) {
                $lesson = M('lesson')->where('id=' . $id)->data($data)->save();
                if ($lesson) {
    ...
    public function status()
    {
        ...
        if ($status == 1) {
           $res = M('lesson')->data(array('status' => 0))->where('id=' . $id)->save();
        }
        if ($status != 1 ) {
            $res = M('lesson')->data(array('status' => 1))->where('id=' . $id)->save();
        }
        if ($res) {
