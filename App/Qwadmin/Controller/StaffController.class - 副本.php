...
class StaffController extends ComController
{
    public function index()
    {
        ...
        $prefix = C('DB_PREFIX');
        if ($order == 'asc') {
            $order = "{$prefix}staff.t asc";
        } elseif (($order == 'desc')) {
            $order = "{$prefix}staff.t desc";
        } else {
            $order = "{$prefix}staff.uid asc";
        }
        if ($keyword <> '') {
            if ($field == 'user') {
                $where = "{$prefix}staff.user LIKE '%$keyword%'";
            }
            if ($field == 'phone') {
                $where = "{$prefix}staff.phone LIKE '%$keyword%'";
            }
            if ($field == 'qq') {
                $where = "{$prefix}staff.qq LIKE '%$keyword%'";
            }
            if ($field == 'email') {
                $where = "{$prefix}staff.email LIKE '%$keyword%'";
            }
        }
        ...
        $count = $user->field("{$prefix}staff.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title")
            ->order($order)
            ->join("{$prefix}auth_group_access ON {$prefix}staff.uid = {$prefix}auth_group_access.uid")
            ->join("{$prefix}auth_group ON {$prefix}auth_group.id = {$prefix}auth_group_access.group_id")
            ->where($where)
            ->count();

        $list = $user->field("{$prefix}staff.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title")
            ->order($order)
            ->join("{$prefix}auth_group_access ON {$prefix}staff.uid = {$prefix}auth_group_access.uid")
            ->join("{$prefix}auth_group ON {$prefix}auth_group.id = {$prefix}auth_group_access.group_id")
            ->where($where)
    ...
    public function edit()
    {
        ...
        if ($uid) {
            //$staff = M('staff')->where("uid='$uid'")->find();
            $prefix = C('DB_PREFIX');
            $user = M('staff');
            $staff = $user->field("{$prefix}staff.*,{$prefix}auth_group_access.group_id")->join("{$prefix}auth_group_access ON {$prefix}staff.uid = {$prefix}auth_group_access.uid")->where("{$prefix}staff.uid=$uid")->find();
    ...
    public function update($ajax = '')
    {
        ...
        if (!$uid) {
            ...
            M('staff')->data($data)->where("uid=$uid")->save();
